...
import io.endrit.sample.utils.{SchemaUtils, SessionUtils}
...
class StreamAggregatorDemo(spark: SparkSession, kafkaBrokers: String, schemaRegistryURL: String) {
  ...
  /**
    ...
    * making sure that the batch contains only data from latest window,
    * then we find the top 10 most viewed pages and user counts  by gender
    * @return
    */
